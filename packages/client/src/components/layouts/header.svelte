...
>
	<div class="h-full container flex items-center gap-5">
		...
		{#if $isAuth$}
			{#if $user$}
				<Dropdown>
					<div slot="overlay" class="w-[250px] pt-2 bg-white dark:bg-neutral-800 shadow rounded-lg">
						<div class="px-4 pb-2">
							<p class="font-medium text-gray-900 dark:text-white truncate">{$user$.name}</p>
							<a
								href={`/${$user$.preferred_username}`}
								class="block text-sm font-medium text-gray-400 truncate hover:text-gray-600 dark:hover:text-gray-200 transition-colors"
				...
				<Button variant="primary">
					<i slot="icon" class="bi bi-pencil" />
					{$t('post')}
				</Button>
			{/if}
		{:else}
